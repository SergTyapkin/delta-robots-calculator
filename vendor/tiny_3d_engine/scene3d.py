...
from .geo_loader import load_geo
from .ply_loader import load_ply
from .part3d import Part3D
...
""")
    ...
    def update(self, name, points, conn, color=None, width=1):
    ...
    def widths(self):
        """Return dictionart of widths

        returns:
        --------
        dict of numbers that is widths.
        """
        list_data = list(self._data.keys())

        name = list_data[0]
        widths = np.full(self._data[name]["n_elmts"], self._data[name]["width"], dtype=np.int16)

        for name in list_data[1:]:
            width = self._data[name]["width"]
            widths = np.concatenate(
                (widths, np.full(self._data[name]["n_elmts"], width, dtype=np.int16)),
                axis=0)
